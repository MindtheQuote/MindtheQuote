...
def normalize_quotation_tags(input_string):
    pattern = r'<(?:\|?quotation(\d+)\|?)>'
    
    def replace_match(match):
        number = match.group(1)
        return f"<|quotation{number}|>"
    
    normalized_string = re.sub(pattern, replace_match, input_string)
...
def process_string(input_dict) -> Optional[Dict[str, str]]:
    ...
    new_dict = {}
    input_str = normalize_quotation_tags(input_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_jsonl", type=str, default="None", help="")
    parser.add_argument("--output_json", type=str, default="None", help="")
